  ...
  server = function(input, output, session) {
    ...
    output$track_pb_overall <- renderPlotly({
      plot_ly(smmy_psc_qtr_smmy_nat, 
        ...
        add_trace(x = ~Quarter, 
                  ...
                  mode = "lines",
                  yaxis = "y2",
                  name = "Percentage of Perception Progression") %>%
        layout(yaxis2 = list(name = "Percentage",
                             overlaying = "y",
                             side = "right"),
    ...
    output$call_bar_score <- renderPlotly({
      plot_ly(tmp_eda_dat_tgt_with_call_all(),
              ...
              type = "bar",
              color = ~hcp.major) %>%
        layout(title = paste("Average Call Number for Physicians with Different Scores in ",
                             input$year5, input$quarter5, sep = ""),
               xaxis = list(title = 'Region'),
    ...
    output$meeting_bar_adv <- renderPlotly({
      plot_ly(tmp_eda_dat_tgt_with_meeting_adv_all(),
              ...
              type = "bar",
              color = ~adv_flag) %>%
        layout(title = paste("Average Meeting Counts for Physicians with Progression VS. Overall in ",
    ...
    output$call_bar_adv <- renderPlotly({
      plot_ly(tmp_eda_dat_tgt_with_call_adv_all(),
              ...
              type = "bar",
              color = ~adv_flag) %>%
        layout(title = paste("Average Call Number for Physicians with Progression VS. Overall in ",
                             input$year6, input$quarter6, sep = ""),
               xaxis = list(title = 'Region'),
               yaxis = list(title = 'Average Call Counts'),
               barmode = "group",
